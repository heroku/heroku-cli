...
async function updateHerokuFormula (brewDir) {
  ...
  const url = `https://cli-assets.heroku.com/heroku-v${SHORT_VERSION}/heroku-v${SHORT_VERSION}.tar.xz`
...
async function updateHomebrew () {
  ...
  const homebrewDir = path.join(tmpDir, 'homebrew-brew')
...
updateHomebrew().catch((err) => {
  console.err(`error running scripts/release/homebrew.js`, err)
  process.exit(1)
})
